...
...
  // To build a json one level at a time, call json() and 
  // supply a char[] & key-value pairs as arguments:
  ...
  // if an argument has no matching pair, is not a fragment 
  // or part of an array, it will be a value to empty key
  ...
  json(buf64, "f[1", 4, floatArray);
  // => [0,0.01,4,1]
  json(buf64, "f[7", 4, floatArray);
  // => [0,0.01,4.44,1.234568]
  json(buf64, "f[h", 4, floatArray);
